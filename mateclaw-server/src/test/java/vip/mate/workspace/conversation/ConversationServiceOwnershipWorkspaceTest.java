...
class ConversationServiceOwnershipWorkspaceTest {
    ...
    // ------------------------------------------------------------------
    // 7. Webchat convs — visible to same-workspace JWT users, blocked cross-workspace
    //    (webchat convs are listed for workspace members; #344 ensures the
    //    direct-access endpoints honor the same boundary)
    // ------------------------------------------------------------------

    @Test
    @DisplayName("webchat conv: visible to a JWT user in the same workspace")
    void webchatConvSameWorkspaceMember() {
        when(conversationMapper.selectOne(any())).thenReturn(conv(WEBCHAT_CONV, "webchat:vA", WS_TENANT_A));
        when(workspaceService.hasPermissionCached(WS_TENANT_A, ALICE_USER_ID, "viewer"))
                .thenReturn(true);

        // Alice is a member of the conv's workspace AND the conv is owned by
        // a shared principal (webchat prefix) — both clauses pass.
        assertThat(service.isConversationOwner(WEBCHAT_CONV, "alice")).isTrue();
    }

    @Test
    @DisplayName("webchat conv: blocked for a cross-workspace user (#344)")
    void webchatConvCrossWorkspaceRejected() {
        when(conversationMapper.selectOne(any())).thenReturn(conv(WEBCHAT_CONV, "webchat:vA", WS_TENANT_A));
        when(workspaceService.hasPermissionCached(WS_TENANT_A, BOB_USER_ID, "viewer"))
                .thenReturn(false);

        // Bob is in tenant B; webchat conv lives in tenant A — rejected.
        assertThat(service.isConversationOwner(WEBCHAT_CONV, "bob")).isFalse();
    }
